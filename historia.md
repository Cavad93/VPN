...
## Запуск 81 — 2026-05-04 (ветка: claude/reduce-vpn-bandwidth-NGVmG)

### Выполнено: Zero-alloc sendmmsg/recvmmsg — embedded VALUE arrays + FD кеш
...
Удалённая ветка добавила `sync.Pool`-based подход (`mmsgStatePool`):
- `Get()` из пула: 1 аллокация при pool miss (каждый goroutine-switch, GC cycle)
- `rawConn.Control()` + closure: 1–2 аллокации на каждый flush (closure heap-escape)
- `SyscallConn()`: 1 аллокация каждый flush (возвращает новый `*rawConn`)
- Итого: **3 аллокации в steady state** (не считая pool miss)

**Решение — три идеи вместо пула:**

1. **Embedded VALUE arrays** — `batchWriterPlatform` и `batchReaderPlatform` хранят mmsghdr/iovec/sockaddr массивы как VALUE поля внутри `batchWriter`/`batchReader`. Аллоцируются ровно один раз (со структурой), никаких `make()` на flush.

2. **FD кеш (send path)** — на первом flush вызываем `rawConn.Control(func(fd) { w.platform.fd = fd })` и сохраняем `fd uintptr`. Все последующие flush вызывают `unix.Syscall6(SYS_SENDMMSG, w.platform.fd, ...)` напрямую — без `SyscallConn()`, без closure.

3. **rawConn кеш (recv path)** — на первом `readPlatform()` кешируем `rawConn syscall.RawConn`. Дальнейшие reads вызывают `r.platform.rawConn.Read(...)` без `SyscallConn()`.
...
type batchWriterPlatform struct {
    ...
    fd        uintptr  // cached once on first flush
    fdReady   bool
}
...
type batchReaderPlatform struct {
    ...
    iovecs    [maxBatchSize]unix.Iovec
    sockaddrs [maxBatchSize]unix.RawSockaddrInet4
    rawConn   syscall.RawConn  // cached once on first read
    rcReady   bool
}
```
...
| Путь | До (pool) | После (embedded+FD) |
|------|-----------|---------------------|
| flushPlatform steady-state | 3 (pool miss + SyscallConn + closure) | **0** |
| flushPlatform first call | 3 | 1 (rawConn.Control closure, one-time) |
| readPlatform steady-state | 3 | **1** (rawConn.Read closure — неизбежно при Go net API) |
| readPlatform first call | 3 | 2 (SyscallConn + Read closure) |

Closure в `rawConn.Read` — архитектурное ограничение Go net API (блокирующий read требует closure для parking). Устранить без замены net.Conn на raw `syscall.Read` невозможно. Документировано в комментарии.

**Интеграция с remote:** удалённая ветка добавила:
- `batchResult.key udpAddrKey` вместо `addr *net.UDPAddr` — сохранено
- `udpAddrKeyFromRawIPv4()` из raw sockaddr — сохранено  
- `r.resBuf [maxBatchSize]batchResult` в `batchReader` — сохранено
- `mmsgStatePool` — заменено embedded arrays

**Обновление тестов:**
- `batch_alloc_test.go` — удалены ссылки на несуществующие поля `results`/`resultAddrs`/`resultIPs` (артефакты предыдущего дизайна)
- `TestBatchWriterZeroAllocsOnFlush` — подтверждено: 0 аллокаций per flush в steady state
- `TestBatchReaderPlatformFields` / `TestBatchWriterPlatformFields` — все поля maxBatchSize
