...
### ФАЗА 7: Мобильные клиенты
- [ ] **ЗАДАЧА 23:** Android клиент — APK на Kotlin, использует tun2socks для перехвата трафика. Дистрибуция через Telegram/прямой APK, без Google Play. `client/android/`
- [ ] **ЗАДАЧА 24:** iOS клиент — Swift приложение с NetworkExtension (Packet Tunnel Provider). Дистрибуция через AltStore (бесплатно, 7-дневная переподпись автоматически по Wi-Fi пока Mac включён). `client/ios/`

### ФАЗА 8: Оптимизация производительности
- [ ] **ЗАДАЧА 25:** Нагрузочное тестирование — скрипт симулирует 4 одновременных клиента (2 Mac + Android + iPhone), замеряет throughput, latency, потери пакетов. `scripts/load_test.py`
- [ ] **ЗАДАЧА 26:** Оптимизация буферов — настройка SO_RCVBUF/SO_SNDBUF, zero-copy sendfile, batching пакетов для максимального throughput при 4 клиентах. `server/transport/optimize.go`

### ФАЗА 9: Обход белых списков через Яндекс.Облако relay

**Архитектура:** `Mac/Android/iPhone → Яндекс.Облако VM (IP в белом списке РКН) → Казахстанский сервер → Интернет`

Яндекс.Облако даёт IP адреса автоматически включённые в белый список РКН (~300-500 руб/мес за минимальную VM).

- [ ] **ЗАДАЧА 27:** Relay сервер на Go — принимает соединение от клиента, пересылает на казахстанский сервер. Поддержка PROXY Protocol v2 для передачи реального IP клиента. Конфиг: IP казахстанского сервера, порт, ключи. `server/relay/relay.go`

- [ ] **ЗАДАЧА 28:** REALITY handshake на Go — форк crypto/tls, проксирование ServerHello от реального сайта (`dl.google.com`), инъекция HMAC токена аутентификации. Пассивный наблюдатель видит легитимный сертификат Google. `server/crypto/reality.go`

- [ ] **ЗАДАЧА 29:** Anti-probe защита — при неверном ключе сервер прозрачно проксирует соединение на настоящий `dl.google.com`. Активный зонд РКН получает реальный ответ Google, сервер не детектируется. `server/crypto/fallback.go`

- [ ] **ЗАДАЧА 30:** Обход 16 КБ троттлинга — разбивка данных на чанки менее 12 КБ, пересоздание TCP сессии до достижения лимита, keepalive пакеты для предотвращения заморозки соединения. `server/transport/antithrottle.go`

- [ ] **ЗАДАЧА 31:** Автоматический выбор маршрута — клиент пробует по порядку: прямое соединение (Казахстан напрямую) → relay Яндекс.Облако → Cloudflare Workers fallback. Прозрачный failover без участия пользователя, выбор запоминается. `client/autoselect.py`

- [ ] **ЗАДАЧА 32:** uTLS браузерный fingerprint — TLS handshake неотличим от Chrome 124 на уровне JA3/JA4 fingerprint. Пакет: `github.com/refraction-networking/utls`. `server/crypto/utls.go`

- [ ] **ЗАДАЧА 33:** Статистический камуфляж — паттерны пакетов имитируют трафик ВКонтакте и Яндекс (размеры пакетов, интервалы, burst pattern). Нейросеть DPI классифицирует как обычный российский веб-трафик. **Важно: имитация VK/Яндекс, НЕ YouTube — YouTube заблокирован в РФ.** `server/transport/mimic.go`

- [ ] **ЗАДАЧА 34:** Установщик relay на Яндекс.Облако — bash скрипт автоматически разворачивает relay сервер на чистой Ubuntu 22.04 VM. Одна команда: `curl -sSL https://... | bash`. Скрипт: установка Go, компиляция relay, systemd unit, генерация ключей, вывод конфига для клиента. `scripts/install_relay.sh`
