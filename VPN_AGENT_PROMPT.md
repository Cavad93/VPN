...
- [x] **ЗАДАЧА 33:** Multi-server поддержка — несколько серверов в конфиге, автопереключение если один упал.
  > Выполнено: `client/multiserver.py` + `client/test_multiserver.py` (42 теста, все pass). Реализованы: `ServerEndpoint(addr, name, connect_timeout)` — одна запись в пуле серверов; `SelectionPolicy` (PRIORITY/ROUND_ROBIN/FASTEST) — стратегия выбора; `MultiServerConfig` — конфигурация менеджера; `ServerSelector` — потокобезопасный выбор сервера по политике с latency probe для FASTEST; `_probe_latency(addr, timeout)` — TCP-замер задержки; `_probe_all(servers, timeout)` — параллельные замеры (daemon thread на каждый сервер, sorted по latency); `MultiServerManager(config, key_pair)` — основной класс: `start()`, `stop()`, `wait_connected(timeout)`, `active_endpoint`, `route_info`; колбэки `on_connected(ep, route)`, `on_disconnected(ep, exc)`, `on_switching(old, new, attempt)`; фоновый health-check поток (threading.Event); exponential backoff между попытками; `create_multi_server_manager(addrs, ...)` — фабричная функция. **Perf 1:** `_get_cached_vpn_config` — кеш VPNConfig объектов по (endpoint_id, key_pair_id) — zero alloc на hot reconnect path. **Perf 2:** `ServerSelector.get()` снимает lock за O(1) (snapshot ссылки на список), вычисляет индекс вне critical section — исключает lock contention при reconnect storms. Запуск: `cd client && python3 -m pytest test_multiserver.py -v`
- [x] **ЗАДАЧА 34:** Split tunneling — выбор каких приложений пускать через VPN, каких напрямую (например банковские приложения напрямую).
  > Выполнено: `client/split_tunnel.py` + `client/test_split_tunnel.py` (54 теста, все pass). Реализованы: `SplitTunnelMode` (EXCLUDE/INCLUDE), `AppRule` (process_name/bundle_id), `SplitTunnelConfig`, `_PrefixIndex` (O(log n) subnet lookup), `_RouteManager` (add/remove routes via system route cmd), `_ProcessMonitor` (pgrep + lsof с PID-кешем), `SplitTunnel` (main class: start/stop/add_app/remove_app/add_bypass_subnet), `create_split_tunnel` (factory). **Perf 1:** prefix-trie (_PrefixIndex) — O(log n) проверка покрытия bypass subnet вместо O(n) линейного скана. **Perf 2:** PID-cache в _ProcessMonitor — lsof (дорогой syscall) пропускается если PID set не изменился с последнего скана. Запуск: `cd client && python3 -m pytest test_split_tunnel.py -v`
