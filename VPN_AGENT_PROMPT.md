...
- [x] **ЗАДАЧА 18:** SNI spoofing — подделка Server Name Indication под легитимные домены.
  > Выполнено: `server/transport/sni.go` + `server/transport/sni_test.go` (26 тестов SNI, 70 тестов transport суммарно, покрытие 90.1%). Реализованы: `SNISelector` — интерфейс выбора домена; `StaticSNI{Domain}` — всегда возвращает фиксированный домен; `RandomSNI{Domains}` + `NewRandomSNI()` — случайный выбор из пула легитимных доменов (www.google.com, www.youtube.com, www.cloudflare.com и др.); `buildSNIExtension(host)` — кодирует TLS server_name extension (RFC 6066 §3); `buildSupportedVersionsExtension()` — extension supported_versions с TLS 1.3; `buildClientHelloWithSNI(sni)` — синтетический ClientHello с SNI + supported_versions extensions; `ExtractSNI(body)` — парсинг SNI из тела ClientHello (для логирования на сервере). В `ObfsConn` добавлены: поле `sniSelector SNISelector`, метод `WithSNI(selector) *ObfsConn` (builder pattern), обновлён `ClientHandshake()` — если sniSelector установлен, отправляет ClientHello с SNI. Запуск: `cd server && go test ./transport/ -v -cover`
