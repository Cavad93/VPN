...
function Invoke-GracefulRestart {
    ...
    # Not a service — just stop the process. Watchdog (watchdog.ps1) handles restart.
    ...
    if ($proc) {
        ...
        Write-Log "INFO" "Process stopped. Watchdog will restart it."
    } else {
        Write-Log "INFO" "No running process found."
    }
}
